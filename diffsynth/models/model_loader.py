...
class ModelPool:
    ...
    def _normalize_model_path(self, path):
        # Keep list input unchanged.
        if isinstance(path, list):
            return path
        if not isinstance(path, str):
            return path

        # If user passes a safetensors index file, expand to shard list.
        ...
        # If user passes one shard (e.g. ...-00001-of-00003.safetensors),
        # expand to all shards with the same prefix.
        shard_match = re.match(r"^(.*)-\d{5}-of-\d{5}\.safetensors$", path)
        if shard_match is not None:
            prefix = shard_match.group(1)
            shards = sorted(glob.glob(prefix + "-*.safetensors"))
            if len(shards) > 0:
                return shards
    ...
    def auto_load_model(self, path, vram_config=None, vram_limit=None, clear_parameters=False):
        path = self._normalize_model_path(path)
