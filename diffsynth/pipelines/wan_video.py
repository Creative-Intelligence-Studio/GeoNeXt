...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        # output
        output_type: Literal["pil", "tensor", "latent"] = "pil",
        zero_noise: bool = False,
        direct_clean_output: bool = False,
    ):
        # Scheduler
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            # Scheduler / direct-clean output mode
            if direct_clean_output:
                # In direct-clean mode, model output is interpreted as clean latents directly.
                inputs_shared["latents"] = noise_pred
                if "clean_reference_latents" in inputs_shared:
                    ref = inputs_shared["clean_reference_latents"]
                    inputs_shared["latents"][:, :, :ref.shape[2]] = ref
            else:
        ...
        is_fused_three_frame = "clean_reference_latents" in inputs_shared or inputs_shared["latents"].shape[2] == 3
        
        if is_fused_three_frame:
            # Strategy: slice -> decode per frame -> concatenate.
            latents = inputs_shared["latents"]
            decoded_frames_list = []
            
            # Iterate over the temporal dimension frame by frame.
            for i in range(latents.shape[2]):
                # Slice one latent frame: [B, C, 1, H, W]
                single_latent = latents[:, :, i:i+1, :, :]
                # Decode a single frame.
                single_video_chunk = self.vae.decode(
                    single_latent, 
                    device=self.device, 
                    tiled=tiled, 
                    tile_size=tile_size, 
                ...
                decoded_frames_list.append(single_video_chunk)
            
            # Concatenate back to [B, 3, T, H, W].
            video = torch.cat(decoded_frames_list, dim=2)
            
        else:
            # Original path: decode all frames in one batch.
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("height", "width", "num_frames", "seed", "rand_device", "vace_reference_image", "zero_noise"),
            output_params=("noise",)
        )

    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, vace_reference_image, zero_noise):
        ...
        if zero_noise:
            noise = torch.zeros(shape, dtype=pipe.torch_dtype, device=pipe.device)
        else:
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    temporal_rope_scale = max(1, int(getattr(dit, "temporal_rope_scale", 1)))
    if temporal_rope_scale > 1:
        t_idx = torch.arange(f, device=x.device) * temporal_rope_scale
        t_idx = t_idx.clamp(max=dit.freqs[0].shape[0] - 1).long()
        temporal_freqs = dit.freqs[0][t_idx]
    else:
